...
- The **write** contract governs the full staff compliance record — the working document a tax professional fills out during the Phase 2 Processing / Due Diligence step. Dedupes by `order_id`; canonical R2 key is `compliance_records/{order_id}.json`; receipts land at `receipts/tmp/compliance-records/{order_id}.json`. Staff-only. D1 projection: `compliance_records` (migration `0042_compliance_records.sql`) — indexes `order_id` (PK), `account_id`, `servicing_professional_id`, `record_status`. R2 remains authoritative.
...
| GET   | `/v1/admin/scale/prospects/search` | Session-auth proxy for the FOIA prospect search. Same logic as `GET /v1/scale/prospects/search` (SCALE_API_KEY auth) but uses `vlp_session` cookie + admin email check. Query params: `q`, `limit` (1-500, default 50), `offset`. Used by the `/scale/crm/prospects` frontend page. The SCALE_API_KEY route remains for CLI/cron use. |
| GET   | `/v1/admin/scale/prospects/:slug` | Session-auth proxy for the FOIA prospect detail. Same logic as `GET /v1/scale/prospects/:slug` (SCALE_API_KEY auth) but uses `vlp_session` cookie + admin email check. Returns full prospect record + `emails_sent[]` from send queue archives. Used by the `/scale/crm/prospects/[slug]` frontend page. |
...
- **Search route:** `GET /v1/scale/prospects/search?q=<query>&limit=50&offset=0` — `Authorization: Bearer <SCALE_API_KEY>`. Filters the FOIA master JSONL by name, firm, city, email, or domain (case-insensitive). Returns paginated prospect summaries with slug, contact info (email masked), email_status, campaign assignment, and a constructed LinkedIn search URL. Performance: parses full 88K NDJSON in-memory on each call (admin-only, acceptable for now).
- **Detail route:** `GET /v1/scale/prospects/:slug` — `Authorization: Bearer <SCALE_API_KEY>`. Returns the full FOIA record for a prospect identified by slug (`dailyMakeSlug(first, last, city, state)`), plus enrichment timestamps and `emails_sent[]` array built by scanning the campaign-specific send queue and sent archives. Each entry in `emails_sent` includes email_number (1-6), subject, body, sent_at, and campaign. Also returns a LinkedIn search URL constructed from `Full_Name + City + Profession`.
...
```
empty
  ↓ (find-emails @ 06:00 UTC — discovery only)
unverified
  ↓ (validate-emails @ 08:00 UTC — deliverability verdict)
valid | invalid | risky | unknown | error
```

Dead ends from find-emails (`no_mx`, `no_patterns`, `invalid`) never reach
validate-emails — those rows have no `email_found` value and are skipped by
the validation eligibility filter.

Tracking columns on each row:
- `email_found` — discovered address (written by find-emails)
- `email_status` — lifecycle state per the diagram above
- `email_found_at` — ISO timestamp of discovery
- `email_discovery_method` — winning pattern (`first`, `first.last`, etc.)
...
- **Discovery-only semantics:** when a candidate is accepted, `email_status` is always written as `"unverified"` — Reoon's Power mode verdict is not recorded here. The job of this cron is to answer "what is the email address?" not "is it deliverable?" The 08:00 UTC validate-emails cron owns the deliverability verdict. Rows with no MX records are terminal dead ends and get `email_status = "no_mx"` with no `email_found`.
- **Columns written:** `email_found`, `email_status` (`unverified` on success; `no_mx` | `no_patterns` | `invalid` on failure), `email_found_at`, `email_discovery_method`
- **R2 log key:** `vlp-scale/logs/find-emails-{YYYY-MM-DD}.json`
- **Secret:** `REOON_API_KEY` (also used by enrichment + validate-emails crons)
