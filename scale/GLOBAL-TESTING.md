...
5. ____https://virtuallaunch.pro/affiliate: 1) Page does not load, shows error: Application error: a server-side exception has occurred while loading virtuallaunch.pro (see the server logs for more information).
Digest: 2486881340
6. ____https://virtuallaunch.pro/receipts: 1) Still need to text with a test or live receipt
7. ____https://virtuallaunch.pro/token-usage: 1) Not showing usage history although tokens have been used
...
4. ____https://taxmonitor.pro/dashboard: 1) The tax pro-specific dashboard is incorrectly nested inside the taxpayer/public dashboard, there should only the taxpayer dashboard, the tax pro-specific dashboard should only be avaiable through VLP, 2) Token Balance prints "NaN" versus 0 or the actual amount
...
5. ___https://transcript.taxmonitor.pro/app/dashboard/: 1) Email report link to client doesn't work as intended, prehaps we should remove this entirely (do as is best recommended)
6. ___https://transcript.taxmonitor.pro/app/reports/: 1) No reports are loading although reports have been saved/printed to PDF
